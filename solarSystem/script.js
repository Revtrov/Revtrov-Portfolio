...
const scene = new THREE.Scene(),
    camera = new THREE.PerspectiveCamera(70, window.innerWidth / window.innerHeight, 1, 300000000000),
...
const sky = new THREE.Mesh(
    new THREE.SphereGeometry(300000000000, 16, 16),
...
function addSphere(textureURL, radius, x, y, z) {
    ...
    const sphere = new THREE.Mesh(
        new THREE.SphereGeometry(radius, 265, 265),
...
const debugOn = (array) => {
    for (let i = 0; i < array.length; i++) {
        let axesHelper = new THREE.AxesHelper(57900000);
        axesHelper.position.x = array[i].position.x;
        axesHelper.position.y = array[i].position.y;
        axesHelper.position.z = array[i].position.z;
        scene.add(axesHelper);
        axisHelpers.push(axesHelper);
    }

};
const debugOff = () => {
    for (let i = 0; i < axisHelpers.length; i++) {
        scene.remove(axisHelpers[i]);
    }
    axisHelpers = [];
};
...
let touch = 0;
window.addEventListener('touchstart', () => { touch = 1; });
window.addEventListener('touchend', () => { touch = 0 });
