...
class Object {
    ...
    move() {
        this.x -= getRandomInt(0, 2);
        this.x += getRandomInt(0, 2);
        this.y += getRandomInt(0, 2)
        this.y -= getRandomInt(0, 2)
    }
    draw(color, lineThickness) {
        ...
        ctx.arc(this.x * euclidAlg(), this.y * euclidAlg(), this.r, 0, 2 * Math.PI);
...
let reset = (objs) => {
    ...
    for (let i = 0; i < objs.length; i++) {
        ...
        //objs[i].objColor = `rgba(${/*newR*/0}, 255, 0)`;
...
for (let i = 0; i < objCount; i++) {
    objects.push(new Object( /*sizes[getRandomInt(0, sizes.length)]*/ getRandomInt(1, 1), 20, 0, 0, 0, 0, colors[colArray[i]], getRandomInt(0, 2)))
}
...
let loop = setInterval(() => {
    ...
    for (let i = 0; i < objects.length; i += 1) {
        objects[i].draw("rgb(0, 255, 0)", 0.1)
        objects[i].move()
    }
}, )